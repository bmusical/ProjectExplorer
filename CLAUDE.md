...
| 2026-07-12 | `claude/project-docs-review-153ise` | Full doc-drift pass found one issue (fixed): the Near-term Roadmap still listed "Make the repo public" as an outstanding ship-blocker, but `docs/LAUNCH_CHECKLIST.md` §0/§5.1 confirm this was done 2026-07-08 (commit `68b72aa`) — removed the stale row. Also refreshed `MainForm.cs`'s stated line count (2300+ → 3100+). Everything else (Naming, Key Files, Data Model/Flow, Licensing, Recently Shipped, remaining Roadmap items, README/CHANGELOG) matched the code. Separately, fixed a real bug reported by the user: "Open in External Browser" on a Web Resource whose URL was typed without an `http(s)://` scheme launched it as a local file (`ShellExecute` on a bare string) instead of opening a browser — added `WebResource.TryGetNavigableUri` as the single normalization point, now used by the launch action, the inline preview panel, and `ResourceAvailabilityChecker.CheckWebResourceAsync` (which had the same scheme-less bug causing false "Unavailable" strikethroughs). Also removed Web Resources from the 20-second background auto-retry timer per user request (network/removable drives still auto-retry) — see **Resource Availability** above and `CHANGELOG.md` `[Unreleased]`. Separately, fixed a user-reported dangling reference: `docs/RELEASE.md`'s manual release path and `docs/LAUNCH_CHECKLIST.md` (§5.2 and the Quick Reference) told the developer to run `gh release create --notes-file docs/release-notes/X.Y.Z.md`, but that folder has never existed in the repo's history — nothing ever created it. Also found in the same area: §5.2 and the Quick Reference both still described the pre-automation manual-only flow (commit `updates.xml` *before* creating the release), contradicting the tag-triggered `cut-release.ps1`/`release.yml` flow `docs/RELEASE.md` now recommends and the ordering fix from commit `31cd15c`. Rewrote both checklist sections to match `docs/RELEASE.md`'s current recommended flow, and replaced the dangling notes-file path with a PowerShell snippet that extracts the release notes from `CHANGELOG.md` (mirroring what `release.yml` already does), writing to a new gitignored `release-notes.md` scratch file instead. |
| 2026-07-19 | `claude/sqlite-sql-server-support-6oavcs` | User asked whether `docs/RELEASE.md` looked out of date; found and fixed a real bug, not just staleness. The "recommended: tag-triggered" path's step 4 told the developer to run `.\installer\build-installer.ps1 -Version X.Y.Z -UpdateXml -Sign` to "tag, push, and watch the build for you" — but `build-installer.ps1` does none of that; it's a purely local build script (publish + Inno Setup + optional codesign + optional `updates.xml` bump) with zero git/GitHub CLI interaction. The surrounding prose actually described `cut-release.ps1`'s real behavior (sanity-checks the csproj/`CHANGELOG.md` bump reached `master`, tags, pushes, streams `gh run watch`) — the code sample just named the wrong script. Compounding it, `cut-release.ps1` doesn't even accept `-UpdateXml`/`-Sign` (only `-Version`), and the doc's own next line ("this path skips... code-signing") directly contradicted the `-Sign` flag shown one line above it. `docs/LAUNCH_CHECKLIST.md` already had the correct `cut-release.ps1 -Version X.Y.Z` invocation (no flags) in both its §5.2-equivalent step and its Quick Reference, confirming `RELEASE.md` was the one that had drifted — fixed the step-4 code block and prose to match. Everything else in `RELEASE.md` (the manual path's `build-installer.ps1` usage, the changelog-extraction snippet, the Notes section) checked out against the actual scripts. |
...
| Project | Role |
|---|---|
| `ProjectExplorer.Core` | Models, `ProjectManager` service, `IProjectRepository` interface, SQLite persistence (default) with legacy JSON persistence kept for one-time migration, `LicenseManager` |
...
All children inherit from `ProjectChild` (Id, ParentId, SortOrder, Metadata dictionary). The `ChildType` enum discriminator drives polymorphic deserialization in both `JsonProjectRepository` (manual `JsonNode` traversal, avoiding `System.Text.Json` converter issues) and `SqliteProjectRepository` (a `ChildType` text column plus a recursive rebuild from flat rows, grouped by `ParentId`).

### Data Flow

**SQLite is the default store as of 1.1.0.** User action in `MainForm` → `ProjectManager` (async CRUD) → `SqliteProjectRepository` → `%APPDATA%\ProjectExplorer\projects.db`. `SaveProjectAsync` only replaces the rows belonging to the one project being saved (a transactional delete+reinsert of that project's `ProjectChildren` rows) rather than rewriting every project in storage — this is the actual fix for the slowdown a large/deep nest caused under the old JSON store, which rewrote the *entire* file on every single edit. `SaveAllAsync` (used only by top-level project reordering) is still a full transactional replace, matching the old JSON semantics, since that path is rare enough that the cost doesn't matter.

`ProjectStoreMigrator.ResolveRepository(storageDir)` (called once from `Program.cs`, before `ProjectManager` is constructed) decides which repository to hand back: if `projects.db` already exists, use it as-is; if only a legacy `projects.json` exists (pre-1.1.0 install), migrate it and rename the JSON file to `projects.json.migrated` — kept as a safety copy, never read again; if migration throws for any reason (corrupt JSON, disk error), this run falls back to `JsonProjectRepository` untouched, so a user never sees their tree appear to vanish, and migration is simply retried on the next launch. `JsonProjectRepository` itself is otherwise unchanged and kept solely to serve this migration path and legacy-install fallback.

**Migration is written to be crash-safe, not just exception-safe** — this was a real bug, not a hypothetical: an early version wrote the new database directly to `projects.db` from the start (`SqliteProjectRepository`'s constructor creates the file immediately via `EnsureSchema()`, before any data is copied in), so a user whose first run was interrupted anywhere between that constructor call and the final `File.Move` of `projects.json` — an app crash, a force-kill, closing it too early — ended up with a `projects.db` file that existed but was empty or partial. Since `ResolveRepository` trusts `File.Exists(dbPath)` as "already migrated" (line 1 of the check above), that partial file was trusted forever, permanently orphaning the untouched `projects.json` the app no longer looked at — from the user's perspective, their entire project tree just vanished. The fix: migration now writes into a temp file (`projects.db.migrating-<guid>`), calls `SqliteProjectRepository.Checkpoint()` (forces all WAL data into the main file and truncates the WAL, so the temp file is guaranteed self-contained), and only becomes `projects.db` via one atomic `File.Move` — so the real path is never observable in a partially-migrated state, whatever kind of interruption occurs. `ProjectStoreMigratorTests` has regression coverage for a leftover temp file from an interrupted prior run (must not block a clean retry) and for no leftover temp/`-wal`/`-shm` files after either a success or a failure.

License state is stored separately at `%APPDATA%\ProjectExplorer\license.json` via `LicenseManager`; tree expand/select state at `uisettings.json`; window bounds at `appsettings.json` via `AppSettingsManager`.

`ProjectManager` loads the entire project list into memory once, in `InitializeAsync()` at
startup, and never refreshes it from disk afterward — there's no file watcher or polling. A
second running copy of the app, with its own independent in-memory copy of the same data, could
silently discard the first copy's edits this way (whichever repository is behind `ProjectManager`)
— which is why single-instance enforcement (below) is mandatory rather than a user choice.

### Key Files

- `src/ProjectExplorer.Core/Services/ProjectManager.cs` — all business logic; start here for any feature work
- `src/ProjectExplorer.Core/Services/SqliteProjectRepository.cs` — the default `IProjectRepository`; Dapper + raw SQL against `projects.db` (two tables: `Projects`, `ProjectChildren` — see Data Flow above), deliberately no ORM, mirroring `JsonProjectRepository`'s own "no magic" style. Schema/SQL kept portable so a future `SqlServerProjectRepository` can implement the same interface without rework (not built yet — SQLite only as of 1.1.0)
- `src/ProjectExplorer.Core/Services/ProjectStoreMigrator.cs` — one-time JSON→SQLite migration on first launch after upgrading; see Data Flow above
- `src/ProjectExplorer.Core/Services/JsonProjectRepository.cs` — legacy persistence; manual JSON node traversal for polymorphic deserialization. No longer the default, but still used by `ProjectStoreMigrator` to read a pre-1.1.0 install's data and as the fallback if migration fails
- `src/ProjectExplorer.Core/Services/SearchService.cs` / `Models/SearchResult.cs` — Comprehensive Search: in-memory recursive scan over `ProjectManager.Projects` (name/description/path/URL/metadata, case-insensitive substring, one result per matching node); see Recently Shipped below
- `src/ProjectExplorer.WinForms/Forms/SearchForm.cs` — modeless search window (toolbar magnifier / `Ctrl+F`), live-filters via `SearchService` and jumps to a result via `MainForm.SelectTreeNodeByTag`
...
- `src/ProjectExplorer.WinForms/Forms/MainForm.cs` — 3300+ line main window; TreeView (left) drives ListView (right) with navigation history stacks, drag-and-drop reparenting/reordering/conversion, and unified context menus
...
- `src/ProjectExplorer.Core/Sharing/` and `src/ProjectNest.Server/` — phase-1 Nest Egg exchange (short code). The sharing database is SQL Server (`Sql/001_CreateSharingDatabase.sql`, database `ProjectNestSharing`). See `docs/SHARING_PHASE1.md`
...
- `tests/ProjectExplorer.Tests/ProjectManagerTests.cs` — xUnit tests for CRUD operations and tree traversal (against `JsonProjectRepository`, still exercised directly as the migration source)
- `tests/ProjectExplorer.Tests/SqliteProjectRepositoryTests.cs` — xUnit tests for `SqliteProjectRepository`: round-trip fidelity for all four child types, nested collections, metadata, sort-order, partial-save isolation between projects, and delete cascade
- `tests/ProjectExplorer.Tests/ProjectStoreMigratorTests.cs` — xUnit tests for `ProjectStoreMigrator`: fresh install, JSON→SQLite migration + rename, idempotent re-resolve, and the corrupt-JSON fallback path
- `tests/ProjectExplorer.Tests/SearchServiceTests.cs` — xUnit tests for `SearchService`'s field matching (name/description/path/URL/metadata), case-insensitivity, recursion, and one-result-per-node behavior
...
The pattern established by `WebResource` and `FileReference` is: add model → add `ChildType` enum value → handle in `SqliteProjectRepository`'s row mapping and `JsonProjectRepository`'s deserialization switch → add a case in `SearchService.SearchChildren` → add `ProjectManager` CRUD methods → wire up context menu and dialog in `MainForm` → add tests.
...
as `Unavailable`: only a confidently-broken HTTP error response does — a 404, a 410, a 5xx, etc.
...
Two more sources of false positives were found 2026-07-14, after a user reported *every* WebResource
rendering broken: (1) the request carried no `User-Agent` header — `HttpClient` sends none by
default — which plenty of WAFs/anti-bot layers (Cloudflare, Akamai, etc.) treat as a bot signature
and answer with a blanket 403 even though the page is fine; `CheckWebResourceAsync` now sends a
spoofed desktop-Chrome UA (`BrowserUserAgent`). (2) 401/403/429 responses were being treated the
same as a 404/500 ("confirmed broken"), but those three status codes far more often mean the
automated check got blocked or rate-limited, or hit an auth wall the user's own logged-in browser
would sail through, than that the resource is actually gone — `ClassifyResponse` now reports those
three as `Unknown` (same as a connection failure) rather than `Unavailable`; only a clearer error
(404, 410, 5xx, etc.) still counts as confirmed. The strikethrough/grey styling and the
Locate-to-relink flow itself are unchanged and apply uniformly to FolderReference/FileReference/
WebResource — the fix is entirely in making the WebResource check itself more accurate, not in
suppressing the visual for a whole resource type.

`MainForm.cs` is where the session-scoped behavior lives:

- **Cache**: `_availabilityCache` (keyed by `ProjectChild.Id`) survives `RefreshTreeView()` rebuilds. `EnsureAvailabilityChecked` fires a background check the first time a node/item renders; `ForceCheckAvailabilityAsync` bypasses the cache for "Check Availability Now"/"Refresh".
- **Visuals**: only a confirmed `Unavailable` result renders grey + strikethrough (`ApplyAvailabilityStyle`) — `Unknown` (unchecked, or a Web resource whose check was merely inconclusive) renders normally. This applies uniformly to FolderReference/FileReference/WebResource; see the false-positive fixes above (`BrowserUserAgent`, `ClassifyResponse`) for why WebResource's `Unavailable` is now trustworthy enough to keep the same treatment as the other two rather than being excluded from it. The TreeNode tooltip explains *why* an unavailable item is flagged — local-disk unavailability reads as "likely moved or deleted", network/removable as "may be temporarily disconnected", web as "the site returned an error" (`DescribeUnavailable`). `UpdateAvailabilityVisuals` patches an already-rendered node/item in place (no tree rebuild) so the 20-second retry timer doesn't disrupt scroll position or expand state. The strikethrough `Font` objects (`_unavailableTreeFont`/`_unavailableListFont`) are deliberately never explicitly disposed — they stay assigned to live `TreeNode.NodeFont`/`ListViewItem.Font` for as long as the app runs, and disposing them in `OnFormClosing` while nodes still referenced them caused a `Font.ToHfont()` crash in `TreeView.CustomDraw` on a late repaint during shutdown; the OS reclaims them at process exit regardless.
- **Auto-retry (`NetworkOrRemovable` only)**: `NetworkOrRemovable` resources that are currently `Unavailable` OR `Unknown` get re-checked by `_availabilityRetryTimer` (`RecheckUnavailableResourcesAsync`) — a missing local-disk file was moved or deleted, not disconnected, so polling it is pointless and it's left alone (user must use "Locate..."). **Web resources are deliberately excluded from this timer** — polling a URL in the background every 20 seconds is unwanted network chatter for something the user can just click "Refresh" on, so a WebResource is only ever (re-)checked when it's first shown (`EnsureAvailabilityChecked`) or explicitly refreshed (`ForceCheckAvailabilityAsync`), never on a timer.
- **Context menu** (`AddAvailabilityMenuItems`, called from `AddFolderReferenceMenuItems`/`AddFileReferenceMenuItems`/`AddWebResourceMenuItems`): when unavailable, prepends a disabled "⚠ Unavailable — ..." header, "Check Availability Now", "Locate Folder.../Locate File..." (browse to relink — not offered for WebResource, whose existing "Edit..." already re-targets the URL), and "Stop Auto-Retry"/"Resume Auto-Retry" — the latter is shown only for `NetworkOrRemovable` (persisted per-resource via `ProjectManager.SetChildMetadataAsync` under the `ResourceAvailabilityChecker.SuppressAutoRetryMetadataKey` metadata key — reuses the existing generic `ProjectChild.Metadata` dictionary rather than adding a new persisted field); WebResource never shows it since it has no background retry to stop. WebResource passes `alwaysOfferCheckNow: true, checkNowLabel: "Refresh"`, so a "Refresh" item (same `ForceCheckAvailabilityAsync` action, shorter label since it's now an everyday action rather than a diagnostic one) appears regardless of current status, not just when already flagged unavailable — some sites keep failing the same automated check (e.g. blocking non-browser requests) even though they load fine for the user, so they need a manual way to force a recheck without waiting for the status to already say `Unavailable`. Being always available rather than conditional also just makes it a normal, everyday action — a user can hit "Refresh" on any WebResource for any reason (double-checking before sharing a link, right after editing a URL, or simple curiosity), not only to work around a stuck check.
- **URL normalization**: `WebResource.TryGetNavigableUri(string?, out Uri?)` (Core) resolves a stored URL missing an `http(s)://` scheme (e.g. `example.com`, typed that way in `WebResourceDialog`, which does no scheme normalization on save) to `https://example.com`, and returns `false` for anything with a different explicit scheme (`ftp://...`) or that's unparseable. It's the single source of truth used by `MainForm.LaunchWebResource` (the "Open in External Browser" context-menu action and the preview panel's button both route through it — passing a scheme-less URL straight to `Process.Start(..., UseShellExecute = true)` makes Windows try to open it as a local file, not a browser navigation, which is exactly the bug this helper fixes), by `WebResourcePreviewPanel.Navigate`, and by `ResourceAvailabilityChecker.CheckWebResourceAsync` — so a scheme-less-but-working link is treated identically everywhere: it opens in a browser and never shows the broken/strikethrough styling just for lacking `https://`.
- **Network disclosure**: checking a WebResource's availability makes an outbound HTTP request (GET with `HttpCompletionOption.ResponseHeadersRead`). This happens automatically the first time a WebResource is shown, and again whenever "Refresh"/"Check Availability Now" is used — see the note in `docs/HELP.md`'s "Checking for updates" section, which is the one place the app's "works fully offline" claim needs this caveat.
...
- **F2 rename** — `MainForm.TreeView_KeyDown` handles `Keys.F2` on the selected TreeView node (Project/Collection only — leaf types don't rename here) via inline `TreeNode.BeginEdit()`. `MainForm.ListView_KeyDown` handles the same `Keys.F2` on the selected ListView row (e.g. renaming a top-level Project while its row is selected in the ListView instead of the tree) — the ListView has no inline label editing, so it routes through `RenameViaDialog`, the same dialog the right-click "Rename" menu item already used.
- **Keyboard navigation: Enter-to-open, Del-to-delete** — both `TreeView_KeyDown` and `ListView_KeyDown` now also handle `Keys.Enter` and `Keys.Delete` on the current selection. Enter routes through `ActivateByTag` (extracted from what was previously only `ListView_DoubleClick`'s body, so both input methods share one activation dispatch), with one TreeView-specific addition: on a Project/Collection/root node with children, Enter toggles expand/collapse instead — those types have no distinct "activate" action beyond selection, which already drives the content panel via `AfterSelect`. Delete routes through `DeleteByTagWithConfirmAsync`, which parses the same `Tag*` prefixes and calls one of five extracted `*WithConfirmAsync` methods (`DeleteProjectWithConfirmAsync`, `DeleteCollectionWithConfirmAsync`, `RemoveFolderReferenceWithConfirmAsync`, `RemoveWebResourceWithConfirmAsync`, `RemoveFileReferenceWithConfirmAsync`) — the exact same confirmation dialogs the equivalent right-click menu items already showed, now shared rather than duplicated between the two call sites.
- **Comprehensive Search** — `File ▸ Search...` / `Ctrl+F` / the toolbar magnifier button opens `SearchForm` (modeless), which live-filters via `SearchService.Search` across *every* project, not just the current view or expanded branches — matching name, description, path/URL, and metadata, case-insensitive substring, one result per matching node (first field matched wins, in Name → Description → Path/URL → Metadata priority order). Since `ProjectManager` already holds the full tree in memory (see Data Flow), this is a plain recursive scan, not a new storage dependency or index. Double-click or Enter on a result reconstructs the matching node's `Tag*` string and reuses `SelectTreeNodeByTag` — the same jump mechanism double-click and drag-drop already use — rather than adding a second way to navigate the tree.
...
- **Export All My Data (GDPR-style)** — `File ▸ Export All My Data...` (`MainForm.cs`) zips whichever of `projects.db`, `license.json`, `uisettings.json`, `appsettings.json` (plus any leftover `projects.json`/`.bak`/`.migrated`) currently exist under `%APPDATA%\ProjectExplorer\` into a single file the user picks a destination for, via `UserDataExportService.ExportAll` (Core). This is a one-shot "give me all my data" export to answer a personal-data request or just let a user grab a copy of everything the app holds about them — deliberately *not* an operational backup/restore feature, so there is no matching Import. Distinct from the still-unbuilt "Export / share a project" Medium-term roadmap item below, which is about handing off one project's tree to a colleague, not a full-data dump.
- **SQLite storage backend (default), replacing per-edit full-file JSON rewrites** — `SqliteProjectRepository` (Core) is now the default `IProjectRepository`, storing projects in `%APPDATA%\ProjectExplorer\projects.db` across two tables (`Projects`, `ProjectChildren`) instead of one JSON file. The old `JsonProjectRepository` rewrote *every* project in storage on every single CRUD call (`SaveProjectAsync` was `LoadAllAsync()` + `SaveAllAsync()` under the hood); `SqliteProjectRepository.SaveProjectAsync` instead replaces only the rows for the one project being saved, inside a transaction — cost now scales with the size of the project being edited, not the size of the whole store, which was the actual source of the slowdown a large/deep nest caused. `ProjectStoreMigrator.ResolveRepository` performs a one-time, safety-first migration from a pre-1.1.0 `projects.json` the first time the new version runs — see **Data Flow** above for the full migration/fallback behavior. Built with Dapper + hand-written SQL rather than an ORM, matching `JsonProjectRepository`'s existing explicit-control style; schema kept portable so a `SqlServerProjectRepository` can implement the same `IProjectRepository` interface later (not built yet).
...
| **Cloud sync** | Sync `projects.db` via OneDrive / Dropbox path. Simple if users manage it themselves; complex if we build sync (a SQLite file being written to while syncing is a real risk in a way a JSON file wasn't — would need to be scoped carefully). |
...
| **Packaged nest distribution** | Let one installation bundle an exported nest together with an easy path to installing the program itself (e.g. a copy of/link to the installer), so a recipient who doesn't have the app yet installs it from that same package and finds it already populated the first time they open it — no separate "now go import a file" step. Envisioned mechanism is a **first-run import**: on startup, if there's no existing store yet but a packaged nest file is sitting alongside the exe (or in a known staging location — placed there by the installer, or just copied in by the recipient before/after installing), the app imports it automatically instead of opening empty. Simpler than teaching the installer itself to embed arbitrary data. Aimed at the "Power users / IT professionals" persona (see Target Users) standardizing an onboarding toolkit across a team, or any user handing a ready-to-go nest to someone new. Distinct from the already-scoped **Export / share a project** above (Medium-term), which produces a `.peproj` for a recipient who *already has the app installed* to import by hand. Also distinct from **Export All My Data** (Recently Shipped), a one-user personal-data dump with no import path at all. Needs more scoping (where exactly the staged file lives, collision handling if a store already exists) and real customer signal before committing. |
