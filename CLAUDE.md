...
- **Visuals**: a confirmed `Unavailable` result renders grey + strikethrough (`ApplyAvailabilityStyle`) for FolderReference/FileReference only — `Unknown` (unchecked, or a Web resource whose check was merely inconclusive) renders normally. **WebResource is deliberately excluded from the grey/strikethrough visual entirely**, even when its cached status is `Unavailable`: too many sites return a 4xx/5xx to the automated background check (bot-blocking, WAFs, rate limiting, etc.) while loading fine for the user in a real browser, so the styling was firing on links that weren't actually broken — reported 2026-07-14. The underlying `Unavailable` status is still tracked and still drives the TreeNode tooltip (`BuildAvailabilityTooltip`/`DescribeUnavailable`, "the site returned an error") and the context menu's "⚠ Unavailable" header/"Refresh" item — only the grey `ForeColor`/strikethrough `Font` application is skipped for `ResourceLocationKind.Web`. The TreeNode tooltip explains *why* an unavailable item is flagged — local-disk unavailability reads as "likely moved or deleted", network/removable as "may be temporarily disconnected", web as "the site returned an error" (`DescribeUnavailable`). `UpdateAvailabilityVisuals` patches an already-rendered node/item in place (no tree rebuild) so the 20-second retry timer doesn't disrupt scroll position or expand state. The strikethrough `Font` objects (`_unavailableTreeFont`/`_unavailableListFont`) are deliberately never explicitly disposed — they stay assigned to live `TreeNode.NodeFont`/`ListViewItem.Font` for as long as the app runs, and disposing them in `OnFormClosing` while nodes still referenced them caused a `Font.ToHfont()` crash in `TreeView.CustomDraw` on a late repaint during shutdown; the OS reclaims them at process exit regardless.
