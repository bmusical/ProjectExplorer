...
`ProjectStoreMigrator.ResolveRepository(storageDir)` (called once from `Program.cs`, before `ProjectManager` is constructed) decides which repository to hand back: if `projects.db` already exists, use it as-is; if only a legacy `projects.json` exists (pre-1.1.0 install), migrate it into a fresh `projects.db` and rename the JSON file to `projects.json.migrated` — kept as a safety copy, never read again; if migration throws for any reason (corrupt JSON, disk error), the partially-written `projects.db` is deleted and this run falls back to `JsonProjectRepository` untouched, so a user never sees their tree appear to vanish, and migration is simply retried on the next launch. `JsonProjectRepository` itself is otherwise unchanged and kept solely to serve this migration path and legacy-install fallback.
