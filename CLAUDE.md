...
| **Packaged nest distribution** | Let one installation package a project (or several) into a self-contained "starter kit" that a *new* installation ends up already populated with, rather than the recipient having to separately install the app and then manually import a file. Aimed at the "Power users / IT professionals" persona (see Target Users) standardizing an onboarding toolkit across a team, or any user handing a ready-to-go nest to someone who doesn't have the app yet. Distinct from the already-scoped **Export / share a project** above (Medium-term), which produces a `.peproj` for a recipient who *already has the app installed* to import by hand — this goes further, needing either the installer (`installer/build-installer.ps1`/`ProjectExplorer.iss`) to bundle a seed nest or a first-run flow that detects and imports one automatically. Also distinct from **Export All My Data** (Recently Shipped), which is a one-user personal-data dump with no import path at all. New territory for the installer/first-run pipeline — needs more scoping and real customer signal before committing. |
