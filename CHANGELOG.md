...
- **Share a project with another computer.** File ▸ Share Project… sends the selected project
  to a sharing server and shows a short code. File ▸ Receive Shared Project… on the other
  computer imports a copy. Folder and file paths are copied as stored, not uploaded. The
  sharing server and its database are described in `docs/SHARING_PHASE1.md`.
